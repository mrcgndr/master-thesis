...
\todo{central pixel, FOV einzeichen, vernünftige wahl von Nside zeigen}

\todo{map von neuer rechnung, die hoffentlich noch fertig wird}



\section{Lookup Table Production}

As the detection efficiency maps are ready now, one has to store the information in such a way, that a quick lookup is possible. 

\begin{figure}[H]
	\includegraphics[width=\textwidth]{LUT_transposition_example.pdf}
	\caption[short caption]{\textbf{short caption.} long caption}
