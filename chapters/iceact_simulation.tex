...
\subsection{Impact-location-dependent Detection Efficiency on the Lens}\label{sec:impactlocation}
\todo{blöde Überschrift ...}

Due to their shape, the Winston cones have different maximum incident angles for their two symmetry axes (cf. figure~\ref{iceact:camera:iceact_wico_sketch}). That implicates that there are trajectories with such angles that can not reach the exit window or the SiPM. For the Fresnel lens, this leads to the fact that the detection efficiency may be dependent on the impact position on the lens. To investigate this, a ray tracing simulation is done, where  

\todo{welche Wellenlänge haben die Photonen?}
\todo{Plot mit symmetrischem Pixel der mittig trifft}
\todo{Plot mit symmetrischem Pixel der auf Grenzfläche trifft}
\todo{Plot mit asymmetrischem Pixel der mittig trifft}
\todo{Plot mit asymmetrischem Pixel der auf Grenzfläche trifft}
