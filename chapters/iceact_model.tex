...
\begin{figure}[H]
	\centering
	\saveimageheight[width=0.59\textwidth]{Camera.png}
	...
	\begin{subfigure}[t]{0.39\textwidth}
		...
		\subcaption{Pixel numbering of the camera where the 61 pixels are numbered in a spiral scheme starting with the central pixel.}
...
The step from a G-APD to an SiPM is done if one now combines many G-APDs in a grid structure in parallel connection which is shown in figure~\ref{sipm:circuit_picture}. 
\todo{addition der Amplituden, Proportionalität (siehe Renker, Lorenz), Pulsparametrisierung (FACT paper)}\cite{sipm:renker_lorenz} \cite{sipm:fact_calibration}
...
\begin{figure}[H]
	...
	\caption[Pulse waveforms of an SiPM]{\textbf{Pulse waveforms of an SiPM.} \cite{sipm:hamamatsu_handbook} Multiple measured SiPM waveforms (\enquote{pulses}) with amplitude plotted against time.}
	\label{sipm:pde}
\end{figure}
\todo{pulse plot besser beschreiben}	

Moreover, figure~\ref{sipm:iceact_sipm} shows the SiPM used for the IceAct camera.
\todo{technische Daten IceAct SiPM (auch das mit SensL erwähnen)}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{material/PDE_interpolation.pdf}
	\caption[Photon detection efficiency of SensL J-Series SiPM]{\textbf{Photon detection efficiency of SensL J-Series SiPM.} The data for over voltages of \SI{6.0}{\volt} and \SI{2.5}{\volt} is taken from \cite{sipm:datasheet}, the curve for \SI{5.0}{\volt} -- which is used in the \geant simulation -- is interpolated linearly.}
	\label{sipm:pde}	
\end{figure}
