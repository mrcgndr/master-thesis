...
If a photon now traverses the diode, a possible impact can ionize an atom and thus produce an electron-hole-pair. While the electron is accelerated along the electrical field it can ionize further atoms if the electric field is strong enough -- an avalanche is formed that induces a breakdown of the applied voltage. This working principle is referred to as an \textit{avalanche photo diode} (\textit{APD}). \cite{sipm:renker_lorenz}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{APD_Cell.png}
	\caption[Sketch of a \enquote{$p$-on-$n$} G-APD cell]{\textbf{Sketch of a \enquote{$p$-on-$n$} G-APD cell.} \cite{sipm:renker_lorenz}}
	\label{sipm:apd_cell}	
\end{figure}

To clarify now the \enquote{Geiger-mode} prefix \todo{write}
